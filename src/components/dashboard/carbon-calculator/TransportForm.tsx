...
export const TransportForm = ({
  ...
  const spendFactor = SPEND_FACTORS.upstream_transport;
  ...
  const handleTemplate = () => {
    ...
    // Spend sheet
    const spendHeaders = [`Spend (${currencySymbol}k)`, 'Description', 'Site Name (optional)'];
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (spendSheet) {
          ...
          rows.forEach((row: any) => {
            const spendKey = Object.keys(row).find(k => k.toLowerCase().includes('spend'));
            const s = parseFloat(spendKey ? row[spendKey] : 0);
            if (!s) return;
  ...
  const handleExport = () => {
    ...
    if (spendEntries.length > 0) {
      const rows = spendEntries.map(e => {
        const site = e.siteId ? sites.find(s => s.id === e.siteId) : null;
        return {
          [`Spend (${currencySymbol}k)`]: e.quantity,
          'Description': e.description,
          'tCO₂e': parseFloat(e.tco2e.toFixed(6)),
          'Site': site?.name || 'Global',
        };
      ...
      {method === 'spend' && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Factor: {spendFactor.factor} tCO₂e per {currencySymbol}k spent ({spendFactor.source})
            </CardDescription>
          </CardHeader>
          <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-2">
              <Label>Total Spend ({currencySymbol} thousands)</Label>
            ...
            {spend && (
              <p className="col-span-full text-xs text-muted-foreground">
                = {(parseFloat(spend) * spendFactor.factor).toFixed(3)} tCO₂e · Factor: {spendFactor.factor} tCO₂e/{currencySymbol}k · Source: {spendFactor.source}
              </p>
            )}
          </CardContent>
        </Card>
