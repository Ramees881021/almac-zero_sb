...
const getCategoryColumns = (currencySymbol: string): Record<string, { headers: string[]; widths: number[] }> => ({
  purchased_goods: { headers: ['supplier_name', 'description', `spend_${currencySymbol}`, 'quantity', 'unit'], widths: [25, 35, 15, 12, 10] },
  capital_goods: { headers: ['supplier_name', 'description', `asset_value_${currencySymbol}`, 'lifespan_years', 'purchase_date'], widths: [25, 35, 18, 15, 15] },
});

const getDefaultColumns = (currencySymbol: string) => ({ headers: ['supplier_name', 'description', `spend_${currencySymbol}`, 'quantity', 'unit'], widths: [25, 35, 15, 12, 10] });
...
export const TemplateManager = ({ category }: TemplateManagerProps) => {
  ...
  const catDef = SCOPE3_CATEGORIES.find(c => c.code === category);
  const label = catDef?.label || category;
  const categoryColumns = getCategoryColumns(currencySymbol);
  const cols = categoryColumns[category] || getDefaultColumns(currencySymbol);
  ...
  const downloadTemplate = () => {
    ...
    const data = [
      cols.headers,
      ...suppliers.map(s => [s.name_display, s.description || '', ...cols.headers.slice(2).map(() => '')]),
    ];
    const ws = XLSX.utils.aoa_to_sheet(data);
    ws['!cols'] = cols.widths.map(wch => ({ wch }));
    const sheetName = (catDef?.label || category).replace(/^\d+\.\s*/, '').slice(0, 31);
    XLSX.utils.book_append_sheet(wb, ws, sheetName);
    XLSX.writeFile(wb, `template_${category}_${suppliers.length}_suppliers.xlsx`);
  ...
  const handleReimport = (file: File) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        const requiredHeaders = ['supplier_name', 'description'];
