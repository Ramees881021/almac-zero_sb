...
export const InvestmentsForm = ({ onAdd, onAddBatch, sites, entries }: InvestmentsFormProps) => {
  ...
  const handleTemplate = () => {
    ...
    const reportedData = [
      ['Investee Name', 'Investee Scope 1+2 (tCO₂e)', `Your Investment (${currencySymbol})`, `EVIC / Total Value (${currencySymbol})`, 'Investment Type', 'PCAF Score', 'Include Scope 3?', 'Investee Scope 3 (tCO₂e)'],
    ...
    // Sheet 3: Spend-Based
    const spendData = [
      ['Investee Name', `Investment Value (${currencySymbol})`, 'Sector Key', 'Investment Type', 'PCAF Score'],
      ...
      try {
        ...
        if (ws1) {
          ...
          rows.forEach(r => {
            ...
            const invest = parseFloat(Object.keys(r).find(k => k.includes('Your Investment')) ? r[Object.keys(r).find(k => k.includes('Your Investment'))!] : 0);
            const totalVal = parseFloat(Object.keys(r).find(k => k.includes('EVIC')) ? r[Object.keys(r).find(k => k.includes('EVIC'))!] : 0);
            ...
            parsed.push({
              categoryCode: 'investments', type: `reported_${invType}`, quantity: invest, unit: currencySymbol, tco2e,
        ...
        if (ws3) {
          ...
          rows.forEach(r => {
            const name = r['Investee Name'];
            const valKey = Object.keys(r).find(k => k.includes('Investment Value'));
            const value = parseFloat(valKey ? r[valKey] : 0);
            ...
            parsed.push({
              categoryCode: 'investments', type: `spend_${invType}`, quantity: value, unit: currencySymbol, tco2e,
