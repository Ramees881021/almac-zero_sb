    ...
    if (activeMethod !== 'supplier') {
      ...
      try {
        ...
        if (data?.results?.[0]) {
          ...
          toast.success(`AI assigned EF: ${result.emission_factor} kg CO₂e/${activeMethod === 'average' ? 'unit' : currencySymbol} (${efSource})`);
        } else {
    ...
    if (aiRows.length > 0) {
      try {
        ...
        aiRows.forEach((row, i) => {
          ...
          const unit = row.method === 'average' ? 'units' : currencySymbol;
