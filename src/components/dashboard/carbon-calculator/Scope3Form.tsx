...
import { Plus, Trash2, Plane, Users, ShoppingCart, Package, Globe, MapPin, Pencil, Check, X, ChevronLeft, ChevronRight, ArrowLeft, Hotel, Loader2, Download, Upload, FileSpreadsheet } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="flex items-end">
            <Button onClick={() => {
              if (spend && factor) {
                onAdd({
                  categoryCode, type: 'spend_based', quantity: parseFloat(spend), unit: `${currencySymbol}k`,
          ...
          {factor && spend && (
            <p className="col-span-full text-xs text-muted-foreground">
              = {(parseFloat(spend) * factor.factor).toFixed(3)} tCO₂e · Factor: {factor.factor} tCO₂e/{currencySymbol}k · Source: {factor.source}
            </p>
          )}
        </div>
      </CardContent>
