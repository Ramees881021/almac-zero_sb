      ...
      reader.onload = async (e) => {
        try {
          ...
          jsonData.forEach((row: any, index: number) => {
            ...
            const scope3Val = s3Key ? parseNumber(row[s3Key]) : null;
            ...
            rows.push({
              ...
              scope_1_emissions: parseNumber(row['Scope 1 Emissions (tCO₂e)']),
              scope_2_location_based: parseNumber(row['Scope 2 Location-based (tCO₂e)']),
              scope_2_emissions: parseNumber(row['Scope 2 Market-based (tCO₂e)']),
              scope_3_emissions: parseNumber(finalScope3),
          ...
          for (const row of rows) {
            ...
            if (fetchError) {
              console.error('Import check error:', fetchError);
              toast.error(`Import failed for year ${row.reporting_year}: ${fetchError.message || 'Unknown error'}`);
              continue;
            }
            ...
            if (!error) {
              ...
              console.error('Import write error:', error);
              toast.error(`Failed to write year ${row.reporting_year}: ${error.message || 'Unknown error'}`);
            }
          }
