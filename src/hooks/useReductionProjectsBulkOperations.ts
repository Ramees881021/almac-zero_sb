...
export const useReductionProjectsBulkOperations = (userId: string | undefined, currencySymbol: string = '£') => {

  const downloadTemplate = useCallback(() => {
    const wb = XLSX.utils.book_new();

    const headers = ['Name', 'Description', 'Scope Type', 'Status', `Project Cost (${currencySymbol})`, 'Annual Emission Savings (tCO₂e)', 'Start Year', 'End Year'];
    const sampleRows = [
    ...
    const refRows: string[][] = [
      ...
      [`Project Cost (${currencySymbol})`, 'Numeric value, e.g. 150000'],
  ...
  const exportData = useCallback(async () => {
    ...
    const rows = data.map((p: any) => ({
      ...
      [`Project Cost (${currencySymbol})`]: p.project_cost,
      'Annual Emission Savings (tCO₂e)': p.annual_emission_savings,
      [`MAC (${currencySymbol}/tCO₂e)`]: p.annual_emission_savings > 0 ? Math.round(p.project_cost / p.annual_emission_savings) : 0,
